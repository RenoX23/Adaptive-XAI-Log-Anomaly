...
def main():
    ...
    threshold = max(0.001, mu - 3 * sigma)
    print(f"Mean normal probability: {mu:.4f}, Std: {sigma:.4f}")
    print(f"Calculated Dynamic Threshold (mu - 3*sigma): {threshold:.4f}")
    
    print("\n6. Evaluating Test Set with Dynamic Thresholding...")
    y_pred = []
    ...
    with torch.no_grad():
        for batch_X, batch_y in test_loader:
            ...
            out = model(batch_X)
            probs = F.softmax(out, dim=1)
            target_probs = probs[torch.arange(len(batch_y)), batch_y].cpu().tolist()
